 ...
 */
	...
	fun setGoal( x: Int, y: Int) {
        try {
			...
			RoomMap.getRoomMap().put(x, y, Box(false, true, false,false))  //set dirty
...
*/
	...
	@JvmStatic fun get_actionSequenceAsString():String{ //funzione per tornare il path in stringa
		return get_actionSequence()?.iterator()?.asSequence()?.toList().toString().replace(" ","")
		.replace(",","")
			.replace("[","")
			.replace("]","")
	}

	@JvmStatic fun updateAfterPath(path : String){ //funzione per aggiornare dopo che il robot ha terminato il path
		var mypath = path
		while (!mypath.equals("")){
			var mov = "" + mypath[0]
	...
	@JvmStatic fun updateAfterFail( path: String ){ //funzione per aggiornare dopo che il robot ha ricevuto stepFail
		var mypath = get_actionSequenceAsString()
		while (!mypath.equals(path)){
			var mov = "" + mypath[0]
			mypath = mypath.substring(1)
			updateMap(mov,"")
		}
...
*/		
    ...
    @JvmStatic fun doMove(move: String, msg: String) {
        val x   = getPosX()  
        val y   = getPosY()  
       ...
       try {
            when (move) {
                ...
                "rightDir" -> map.put(x + 1, y, Box(true, false, false,false))
                "leftDir"  -> map.put(x - 1, y, Box(true, false, false,false))
                "upDir"    -> map.put(x, y - 1, Box(true, false, false,false))
                "downDir"  -> map.put(x, y + 1, Box(true, false, false,false))
		   }//when
        } catch (e: Exception) {
	...
	@JvmStatic fun moveRobotInTheMap(){
		RoomMap.getRoomMap().put(robotState!!.x, robotState!!.y, Box(false, true, true,true))
	}

	@JvmStatic fun moveRobotInTheMapAfterStop(){
		RoomMap.getRoomMap().put(robotState!!.x, robotState!!.y, Box(true, false, false,true))
	}


	@JvmStatic fun cleanMyRoom(){
		RoomMap.getRoomMap().cleanRoom()
	}
...
*/		
 	@JvmStatic fun setObstacleWall(  dir: RobotState.Direction, x:Int, y:Int){
		if( dir == RobotState.Direction.DOWN  ){ RoomMap.getRoomMap().put(x, y + 1, Box(true, false, false,false)) }
		if( dir == RobotState.Direction.RIGHT ){ RoomMap.getRoomMap().put(x + 1, y, Box(true, false, false,false)) }
	}
	...
	@JvmStatic fun setWallDown(dimMapx: Int, y: Int ){
		 var k   = 0
		 while( k < dimMapx ) {
			RoomMap.getRoomMap().put(k, y+1, Box(true, false, false,false))
			k++
		 }		
	}	
	@JvmStatic fun setWallRight(dimMapy: Int, x: Int){
 		 var k   = 0
		 while( k < dimMapy ) {
			RoomMap.getRoomMap().put(x+1, k, Box(true, false, false,false))
			k++
		 }		
	}
